<template>
  <div :style="setBackgroundStyle(getRandomIntValue(1, 1000))" id="tag-page" class="tag-page">
    <div class="tag-page-item">
      <div style="display: none" v-html="pageMap.contentRendered" ref="tagContent" :class="'tagItemContent' + index">
      </div>
      ...
      <div class="tag-page-right tag-page-item-common" id="tag-page-right">
        ...
        <div id="tag-page-right-bottom" v-if="allCategories.length !== 0" class="tag-page-right-bottom">
          <span class="home-menu-ico" style="--homeIcoCode: '\e7b5';color: rgba(98, 182, 203, 0.48);"></span>&nbsp;<span
            :data="allCategories.length" :style="setBackgroundStyle(888)"
            v-for="(item, index) in allCategories">{{ item }}</span>
        </div>
      </div>
    </div>
  </div>
  ...
  computed: {
    ...
    setBackgroundStyle() {
      return (index) => {
        ...
        let style1 = "background-color:rgba(229 252 255 / 54%)"
        return style1
      }
    },
    getTag() {
